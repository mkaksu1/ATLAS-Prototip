...
export default function HomePage() {
  ...
  const [designVersion, setDesignVersion] = useState<1 | 2 | 3>(2);
    ...
    <div className="relative flex min-h-screen flex-col bg-gradient-to-b from-white to-slate-50/30">
      ...
      <header className="relative z-50 flex items-center justify-between gap-2 px-3 py-4 sm:gap-3 sm:px-6 sm:py-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`rounded-lg px-3 py-2 text-xs font-semibold transition-all duration-200 ${
              designVersion === 3
                ? 'bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] text-white shadow-lg'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
          >
            Tasarım 3
        ...
        <div className="relative">
          ...
          {open && (
            <>
              ...
              />
              <div className="fixed left-2 right-2 bottom-2 top-[15vh] sm:absolute sm:left-auto sm:right-0 sm:bottom-auto sm:top-auto z-[110] sm:mt-2 w-auto sm:w-[420px] sm:max-h-[calc(100vh-100px)] animate-in fade-in slide-in-from-bottom-4 sm:slide-in-from-top-2 rounded-3xl border border-slate-200/60 bg-white/95 shadow-2xl backdrop-blur-xl duration-200 flex flex-col overflow-hidden">
                <div className="flex-shrink-0 px-4 pt-4 pb-3 sm:px-6 sm:pt-6 sm:pb-4 flex items-center justify-between border-b border-slate-100">
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {appLinks.map(({ name, Icon, href, color }) => (
                      <Link
                        ...
                        href={href as any}
                        onClick={() => setOpen(false)}
                        className="group flex flex-col items-center gap-2 rounded-2xl p-2.5 sm:p-3 transition hover:bg-slate-50/80"
                      >
                        <div className={`flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${color} shadow-lg shadow-slate-900/10 transition group-hover:scale-110 group-hover:shadow-xl`}>
                ...
                <div className="flex-shrink-0 border-t border-slate-100 px-4 pb-4 pt-3 sm:px-6 sm:pb-6 sm:pt-4">
                  <Link
      ...
      <main className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 py-12">
        {designVersion === 1 ? (
          ...
          <div className="w-full max-w-2xl space-y-10 text-center -mt-16">
          {/* Logo */}
          ...
          <div className="w-full max-w-2xl space-y-10 text-center">
            ...
            {/* Premium App Grid - Horizontal Scroll with All Apps */}
            <div className="animate-in fade-in duration-700 delay-300 pt-6">
              <div className="relative group">
                ...
                {canScrollLeft && (
                  <button
                    onClick={scrollLeft}
                    className="absolute left-0 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg border border-slate-200 text-slate-700 transition-all duration-200 hover:scale-110 hover:shadow-xl hover:bg-gradient-to-r hover:from-[#0B1B3D] hover:to-[#2d4a7c] hover:text-white hover:border-transparent"
                  >
                ...
                <div 
                  ...
                  className="flex gap-4 overflow-x-auto px-2 py-2 scrollbar-hide"
                  style={{ 
                    scrollbarWidth: 'none',
                ...
                >
                  {appLinks.map(({ name, Icon, href }) => (
                    <Link
                      key={name}
                      href={href as any}
                      className="group/app flex flex-col items-center gap-2 rounded-2xl p-3 transition hover:bg-[#0B1B3D]/5 flex-shrink-0"
                    >
                      <div className="relative">
                        <div className="absolute inset-0 rounded-2xl bg-[#0B1B3D]/20 opacity-0 blur-xl transition-opacity duration-300 group-hover/app:opacity-100" />
                        <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-r from-[#0B1B3D] to-[#2d4a7c] shadow-lg shadow-[#0B1B3D]/20 transition-all duration-200 group-hover/app:scale-110 group-hover/app:shadow-xl">
                          <Icon className="h-7 w-7 text-white" />
                        </div>
                      </div>
                      <span className="text-center text-xs font-medium leading-tight text-slate-700 w-16">
                        {name}
                      </span>
                    </Link>
                  ))}
                ...
                {canScrollRight && (
                  <button
                    onClick={scrollRight}
                    className="absolute right-0 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg border border-slate-200 text-slate-700 transition-all duration-200 hover:scale-110 hover:shadow-xl hover:bg-gradient-to-r hover:from-[#0B1B3D] hover:to-[#2d4a7c] hover:text-white hover:border-transparent"
                  >
          ...
          // TASARIM 3 - Premium Tasarım (Tasarım 1 Logo Renkleri)
          <div className="w-full max-w-2xl space-y-10 text-center">
            {/* Refined ATLAS Logo - Tasarım 1 renkleri */}
            <div className="space-y-4 sm:space-y-5 animate-in fade-in duration-700 delay-100">
              <div className="relative">
                {/* Dynamic glow with gradient */}
                <div className="absolute inset-0 bg-gradient-to-r from-[#0B1B3D]/20 to-[#2d5a9f]/20 blur-[80px]" />
                
                {/* Main Logo with smooth gradient */}
                <h1 className="relative text-6xl sm:text-[5.5rem] md:text-[7rem] font-black italic tracking-tight bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] bg-clip-text text-transparent leading-none drop-shadow-2xl">
                  ATLAS
                </h1>
              </div>
            </div>
            ...
            <div className="animate-in fade-in duration-700 delay-200 mx-auto w-full">
              <form onSubmit={(e) => {
                ...
                <div className="group relative overflow-hidden rounded-full border-2 border-slate-200/60 bg-white shadow-xl shadow-slate-900/10 ring-1 ring-slate-100 transition hover:border-[#2d5a9f]/30 hover:shadow-2xl">
                  {/* Hover gradient overlay */}
                  <div className="absolute inset-0 bg-gradient-to-r from-[#0B1B3D]/[0.03] to-[#2d5a9f]/[0.03] opacity-0 transition duration-500 group-hover:opacity-100" />
                  
                  <div className="relative flex items-center gap-3 px-4 py-3 sm:gap-4 sm:px-7 sm:py-5">
                    <MagnifyingGlassIcon className="h-5 w-5 flex-shrink-0 text-slate-400 transition-all duration-300 group-hover:scale-110 group-hover:text-[#2d5a9f]" />
                    <input
                      type="text"
                      value={searchValue}
                      onChange={(e) => setSearchValue(e.target.value)}
                ...
                <div className="mt-6 sm:mt-8 flex justify-center gap-3 sm:gap-4">
                  <button type="submit" className="group relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 px-4 py-2.5 sm:px-7 sm:py-3.5 text-xs sm:text-sm font-semibold text-slate-800 shadow-lg shadow-slate-900/10 ring-1 ring-slate-200/50 transition hover:shadow-xl hover:shadow-slate-900/20 active:scale-95">
                    <span className="relative z-10">ATLAS'ta Ara</span>
                    <div className="absolute inset-0 bg-gradient-to-r from-[#0B1B3D]/5 to-[#2d5a9f]/5 opacity-0 transition group-hover:opacity-100" />
                  </button>
                  <button 
                    type="button"
                    onClick={() => {
                    ...
                    className="group relative overflow-hidden rounded-xl bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] px-4 py-2.5 sm:px-7 sm:py-3.5 text-xs sm:text-sm font-semibold text-white shadow-lg shadow-[#0B1B3D]/25 ring-1 ring-white/10 transition hover:shadow-xl hover:shadow-[#0B1B3D]/35 active:scale-95"
                  >
                    <span className="relative z-10 flex items-center gap-2">
                      <SparklesIcon className="h-4 w-4 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12" />
                      ATLAS.AI'ya Sor
            ...
            <div className="animate-in fade-in duration-700 delay-300 pt-6">
              <div className="relative group">
                ...
                {canScrollLeft && (
                  <button
                    onClick={scrollLeft}
                    className="absolute left-0 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg border border-slate-200 text-slate-700 transition-all duration-200 hover:scale-110 hover:shadow-xl hover:bg-gradient-to-r hover:from-[#0B1B3D] hover:to-[#2d5a9f] hover:text-white hover:border-transparent"
                  >
                    <ChevronLeftIcon className="h-5 w-5" />
                  </button>
                )}
                ...
                >
                  {appLinks.map(({ name, Icon, href }) => (
                    ...
                    >
                      <div className="relative">
                        <div className="absolute inset-0 rounded-2xl bg-[#0B1B3D]/20 opacity-0 blur-xl transition-opacity duration-300 group-hover/app:opacity-100" />
                        <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] shadow-lg shadow-[#0B1B3D]/20 transition-all duration-200 group-hover/app:scale-110 group-hover/app:shadow-xl">
                          <Icon className="h-7 w-7 text-white" />
                        </div>
                      </div>
                      <span className="text-center text-xs font-medium leading-tight text-slate-700 w-16">
                ...
                {canScrollRight && (
                  <button
                    onClick={scrollRight}
                    className="absolute right-0 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg border border-slate-200 text-slate-700 transition-all duration-200 hover:scale-110 hover:shadow-xl hover:bg-gradient-to-r hover:from-[#0B1B3D] hover:to-[#2d5a9f] hover:text-white hover:border-transparent"
                  >
                    <ChevronRightIcon className="h-5 w-5" />
                  </button>
                )}
            ...
            <div className="animate-in fade-in duration-700 delay-400 space-y-2">
              ...
              <div className="flex items-center justify-center gap-2 text-xs text-slate-500">
                <span className="rounded-full bg-[#0B1B3D]/8 border border-[#0B1B3D]/15 px-3 py-1 font-medium transition-all duration-200 hover:scale-110 hover:bg-[#0B1B3D]/12 hover:shadow-md cursor-default">
                  <span className="bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] bg-clip-text text-transparent font-semibold">Yerli</span>
                </span>
                <span className="rounded-full bg-[#0B1B3D]/8 border border-[#0B1B3D]/15 px-3 py-1 font-medium transition-all duration-200 hover:scale-110 hover:bg-[#0B1B3D]/12 hover:shadow-md cursor-default">
                  <span className="bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] bg-clip-text text-transparent font-semibold">Güvenli</span>
                </span>
                <span className="rounded-full bg-[#0B1B3D]/8 border border-[#0B1B3D]/15 px-3 py-1 font-medium transition-all duration-200 hover:scale-110 hover:bg-[#0B1B3D]/12 hover:shadow-md cursor-default">
                  <span className="bg-gradient-to-r from-[#0B1B3D] to-[#2d5a9f] bg-clip-text text-transparent font-semibold">Güçlü</span>
                </span>
              </div>
            </div>
          </div>
